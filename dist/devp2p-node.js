...
const devp2p = require('ethereumjs-devp2p'); // const LRUCache = require('lru-cache');
...
const rlp = require('rlp-encoding'); // const Buffer = require('safe-buffer').Buffer;
...
class DevP2PNode {
  ...
  broadcast(msg, peerId, code = 1) {
    if (peerId) {
      ...
      for (let peerId of (0, _keys.default)(this.peers)) {
        let peer = this.peers[peerId];
        peer.shh.sendRawMessage(code, msg);
      }
    }
  ...
  _startRLPX() {
    ...
    this.rlpx.on('peer:added', peer => {
      ...
      shh.events.on('message', (message, peer) => {
        ...
        if (this.tracker.exists(message, 'devp2p')) return; // Verifying if old message is sent by trusted peer
        ...
        this.tracker.push(message, 'devp2p'); // Broadcast received message again.
    ...
    this.rlpx.on('peer:removed', (peer, reasonCode, disconnectWe) => {
      ...
      if (staticNode) {
        ...
        this.rlpx.connect({
          id: staticNode.id,
          address: staticNode.address,
          port: staticNode.port
        });
      }
...
//# sourceMappingURL=devp2p-node.js.map