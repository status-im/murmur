  ...
  return new _promise.default(function (resolve, reject) {
    const nodeHandler = (err, peerInfo) => {
      ...
      p2pNode.handle('/ethereum/shh/6.0.0', (protocol, conn) => {
        pull(conn, pull.map(v => rlp.decode(Buffer.from(v.toString(), 'hex'))), drain(messages => {
          conn.getPeerInfo((err, peerInfo) => {
            ...
            self.tracker.push(message, 'libp2p'); // Broadcast received message again.
...
class LibP2PNode {
  ...
  _startDiscovery() {
    ...
    this.node.on('peer:connect', peer => {
      ...
      this.peers[peer.id.toB58String()] = {
        peer
      };
    });
