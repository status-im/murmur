...
class Manager {
  ...
  listenToProviderEvents() {
    this.provider.events.on('post', (payload, cb) => {
      ...
      if (pubKey && symKeyID) return cb("Either symKeyID or pubKey should be specified, not both"); // Set symmetric key that is used to encrypt the message
      ...
      const dispatchEnvelope = (err, encryptedMessage) => {
        ...
        if (targetPeer) {
          // Mailserver request
          if (devp2p) devp2p.broadcast(p, targetPeer.toString('hex'), 126); // TODO: libp2p mailserver
        } else {
          if (devp2p) {
            devp2p.broadcast(p);
    ...
    this.provider.events.on("addPeer", (url, cb) => {
      ...
      const port = ipInfo[1];
      this.getNode('devp2p').addStaticPeer({
        id,
        address,
        port
      }, (err, data) => {
    ...
    this.provider.events.on("requestMessages", (minPow, message, cb) => {
      ...
      const now = (0, _parseInt2.default)(new Date().getTime() / 1000, 10);
      ...
      const envelope = {
        ...
        topic: "0x00000000",
        powTime: 1,
        //  TODO: If using default time of 5 secs, peer will disconnect. PoW needs to happen in a separate thread
        powTarget: minPow,
        payload: payload,
        targetPeer: peerId
      };
    ...
    this.provider.events.on("generateSymKeyFromPassword", (password, cb) => {
      ...
      pbkdf2(password, "", 65356, constants.symKeyLength, 'sha256', (err, derivedKey) => {
        ...
        this.keys[id] = {
          symmetricKey: "0x" + derivedKey.toString('hex')
        };
        cb(null, id);
      });
    });
  ...
  sendEnvelopeToSubscribers(message) {
    if (this.messagesTracker.isSent(message)) return; //console.dir('received message, sending to subscribers...');

    let [expiry, ttl, topic, data, nonce] = message; // Preparing data

    nonce = new Uint64BE(new Big(pow.hexStringToDecString(nonce.toString('hex')))).toBuffer();
    ttl = typeof ttl === 'number' ? ttl : (0, _parseInt2.default)(pow.hexStringToDecString(ttl.toString('hex')), 10);
    ...
    for (let subscriptionId of (0, _keys.default)(topicSubscriptions)) {
      const decryptCB = (err, decrypted) => {
        if (!decrypted) return; // console.dir(decrypted.payload.toString());
