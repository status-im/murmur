...
function hexStringToDecString(s) {
  ...
  s.split('').forEach(function (chr) {
    var n = (0, _parseInt2.default)(chr, 16);
...
const powToFirstBit = (pow, data, ttl) => {
  ...
  const res = (0, _parseInt2.default)(Math.ceil(Math.log2(pow * size * ttl)), 10);
...
function ProofOfWork(powTarget, powTime, ttl, topic, data, expiry) {
  ...
  return {
    expiry,
    target,
    nonce: new Uint64BE(resNonce.toString(), 16).toBuffer()
  };
}
