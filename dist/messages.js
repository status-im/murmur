...
const {
  keccak256
} = require("eth-lib/lib/hash"); // const {slice, length, toNumber} = require("eth-lib/lib/bytes");
...
const assignDefined = (target, ...sources) => {
  for (const source of sources) {
    for (const key of (0, _keys.default)(source)) {
...
const decryptAsymmetric = (key, data, cb) => {
  ...
  const ct = data.slice(65, mEnd); // Message Tag
...
const parseMessage = message => {
  let start = 1; // const end = message.length;
...
const buildMessage = (messagePayload, padding, sig, options, cb) => {
  ...
  const flagsLength = 1; // const payloadSizeFieldMaxSize = 4;
  ...
  if (sig !== null) {
    // Sign the message
    if (envelope.readUIntLE(0, 1) & constants.isSignedMask) {
      // Is Signed
