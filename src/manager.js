import {randomBytes, pbkdf2} from "crypto-browserify";
import secp256k1 from "secp256k1";
import {validateDataIntegrity, buildMessage, encryptSymmetric, encryptAsymmetric, decryptAsymmetric, decryptSymmetric} from "./messages.js";
import rlp from "rlp-encoding";
import stripHexPrefix from "strip-hex-prefix";
import {symKeyLength, SHH_P2PREQ, keyIdLength, privKeyLength} from "./constants";
import {ProofOfWork, hexStringToDecString, calculatePoW} from "./pow";
import Big from "big.js";
import {Uint64BE} from "int64-buffer";
import BloomFilterManager, {createBloomFilter, bloomFilterMatch, topicToBloom, bloomFilterAddition} from "./bloom";
...
class Manager {
  ...
  listenToProviderEvents() {
    this.provider.events.on("post", (payload, cb) => {
      let {symKeyID, pubKey, sig, ttl, topic, padding, powTime, powTarget, targetPeer, libp2pPeer} = payload;
      ...
      if (!!symKeyID) {
        ...
        if (
          !validateDataIntegrity(
            Buffer.from(stripHexPrefix(options.symKey.symmetricKey), "hex"),
            symKeyLength
          )
        )
          return cb("Invalid symmetric key");
      } else {
        if (!pubKey) return cb("Pubkey is required");
        if (!stripHexPrefix(pubKey).match(/^[0-9A-Fa-f]{130}$/)) return cb("Invalid pubkey");
      }

      let envelope = buildMessage(messagePayload, padding, sig, options, err => {
      ...
      const dispatchEnvelope = callback => (err, encryptedMessage) => {
        ...
        const powResult = ProofOfWork(powTarget, powTime, ttl, topic, encryptedMessage, expiry);
        ...
        if (targetPeer) {
          if (libp2pPeer && libp2p) {
            libp2p.broadcast(msgEnv, libp2pPeer, SHH_P2PREQ);
          } else if (devp2p) {
            devp2p.broadcast(msgEnv, targetPeer.toString("hex"), SHH_P2PREQ);
          }
        } else {
          if (devp2p) {
      ...
      if (options.symKey) {
        encryptSymmetric(topic, envelope, options, dispatchEnvelope(cb));
      } else {
        encryptAsymmetric(envelope, pubKey, dispatchEnvelope(cb));
      }
    });
    ...
    this.provider.events.on("subscribe", (payload, cb) => {
      ...
      const id = randomBytes(keyIdLength).toString("hex");
    ...
    this.provider.events.on("requestMessages", (minPow, message, cb) => {
      ...
      if (this.getNode("devp2p")) {
        ...
        privateKey = randomBytes(privKeyLength);
      }
    ...
    this.provider.events.on("newKeyPair", cb => {
      ...
      const privKey = randomBytes(privKeyLength);
    ...
    this.provider.events.on("addSymKey", (symmetricKey, cb) => {
      ...
      if (stripHexPrefix(symmetricKey).length / 2 !== symKeyLength) {
    ...
    this.provider.events.on("newSymKey", cb => {
      const id = randomBytes(keyIdLength).toString("hex");
      const symmetricKey = "0x" + randomBytes(symKeyLength).toString("hex");
    ...
    this.provider.events.on("generateSymKeyFromPassword", (password, cb) => {
      ...
      pbkdf2(password, "", 65356, symKeyLength, "sha256", (err, derivedKey) => {
  ...
  sendEnvelopeToSubscribers(envelope) {
    ...
    const nonce = new Uint64BE(new Big(hexStringToDecString(envelope.nonce.toString("hex")))).toBuffer();
    const ttl =
      typeof envelope.ttl === "number"
        ? envelope.ttl
        : parseInt(hexStringToDecString(envelope.ttl.toString("hex")), 10);

    const calculatedPow = calculatePoW(envelope.expiry, ttl, envelope.topic, envelope.data, nonce);
      ...
      try {
        ...
        if (!keyId) {
          ...
          if (this.keys[keyId]) {
            ...
            decryptAsymmetric(key, envelope.data, decryptCB);
          }
        } else {
          let key = Buffer.from(this.keys[keyId].symmetricKey.slice(2), "hex");
          decryptSymmetric(envelope.topic, key, envelope.data, decryptCB);
        }
      } catch (e) {
        //
      }
  ...
  listenToNodeEvents() {
    ...
    if (this.getNode("libp2p")) {
      this.getNode("libp2p").events.on("ready", () => {
        this.isReady("libp2p");
      });
    ...
    this.provider.events.emit("generateSymKeyFromPassword", config.mailserverPassword, (err, symKeyID) => {
      this.getNode("libp2p").events.on("shh_bridge_mailserver_request", (envelope, peerId) => {
        ...
        messages.decryptSymmetric(envelope.topic, key, envelope.data, (err, decrypted) => {
          ...
          if (!this.mailserverSubscriptions[mailserverPeer][peerId])
            this.mailserverSubscriptions[mailserverPeer][peerId] = Buffer.from([]);

          this.mailserverSubscriptions[mailserverPeer][peerId] = bloomFilterAddition(
            this.mailserverSubscriptions[mailserverPeer][peerId],
            bloom
          );
