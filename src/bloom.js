...
export const bloomFilterMatch = (base, searchValue) => {
  if (!base || !searchValue) return true;
  if (base.equals(Buffer.from([])) || searchValue.equals(Buffer.from([]))) return true;
  if (base.length != BloomFilterSize || searchValue.length != BloomFilterSize)
...
export const bloomFilterAddition = (filter1, filter2) => {
...
export const createBloomFilter = message => {
...
export const topicToBloom = topic => {
...
export const topicsToBloom = topics => {
